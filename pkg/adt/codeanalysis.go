...
// abaplintIssueSeverity maps abaplint severity strings to CodeFinding severity levels.
func abaplintIssueSeverity(s string) string {
	switch s {
	case "Error":
		return "high"
	case "Warning":
...
// abaplintRuleCategory maps known abaplint rule keys to CodeFinding categories.
func abaplintRuleCategory(key string) string {
	switch key {
...
// runAbaplintLinter runs the abaplint rule set against source and converts results
// to CodeFinding format. Only rules that are not already covered by the regex engine
// (select_star, hardcoded_credentials, catch_cx_root, commit_in_loop, dynamic_call_no_try)
// are used to avoid duplicates.
func runAbaplintLinter(source string) []CodeFinding {
	// Use a linter with only the new security/quality rules from pkg/abaplint.
	// Rules already covered by the regex engine (obsolete_statement, etc.) are excluded.
	linter := &abaplint.Linter{
		Rules: []abaplint.Rule{
	...
	for _, iss := range issues {
		findings = append(findings, CodeFinding{
			Rule:        iss.Key,
			Category:    abaplintRuleCategory(iss.Key),
			Severity:    abaplintIssueSeverity(iss.Severity),
			Line:        iss.Row,
			EndLine:     iss.Row,
			Match:       iss.Message,
			Description: iss.Message,
			Suggestion:  abaplintRuleSuggestion(iss.Key),
		})
	}
	return findings
}

// abaplintRuleSuggestion returns a human-readable fix suggestion for known rule keys.
func abaplintRuleSuggestion(key string) string {
	switch key {
	case "select_star":
...
// Also runs pkg/abaplint rules for additional coverage.
...
func AnalyzeABAPSource(source string) *CodeAnalysisResult {
	...
	ctx := &scanContext{}
	totalRules := 26 // 21 regex-engine rules + 5 abaplint rules
	...
	// Augment with abaplint-based rules (select_star, hardcoded_credentials,
	// catch_cx_root, commit_in_loop, dynamic_call_no_try).
	// These run on the lexer+parser AST for higher precision than regex.
	findings = append(findings, runAbaplintLinter(source)...)
...
// truncStmt is defined in analysis_types.go
